...
class Aircraft():
    def __init__(self, RateGain, PDgain, B, aoa_lim = [-10, 15], g_lim = [-3, 6], dt = 1/50):
        # States
        ...
        # Extra
        self.dt = dt
        self.mu = np.diag(1e-2*np.array([1,1,1]))
    ...
    def update(self, aircraft_data, stick_in):
        ...
        acc = estimators.accelerationCleaner(R_eb, aircraft_data[8:]*np.array([1, -1, -1]))
        self.acc_raw.append(acc[0])
        self.acc_clean.append(acc[1])
        ...
        # Create setpoint
        self.omega_set.append(controllers.toRate(self.RateGain, sideslip, stick_in))
    ...
    def aoaLimiter(self):
        slope = -250
        self.omega_set[-1][1] = np.clip(self.omega_set[-1][1],
                                 slope*(self.aoa-np.min(self.aoa_lim))**3-np.min(self.aoa_lim),
                                 slope*(self.aoa-np.max(self.aoa_lim))**3+np.max(self.aoa_lim))
    ...
    def adaptiveEffectiveness(self):
        ...
        mu = self.mu / (np.dot(du.T, du)+1e-6)
