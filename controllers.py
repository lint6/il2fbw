...
def LC_pd(error, LCgain, h):
    ...
    vc_d = helper.backwardDiff(error, h) * LCgain[1]
    return vc_p + vc_d
...
def INDI(vc, x, u, Bmat, h):
    ...
    '''
    e_dot = vc - helper.backwardDiff(x, h)
    du = np.array(helper.invert(Bmat) @ e_dot.flatten())
    u += du
    return u

def controlLoopINDI(x, u, input, h, LCgain, Bmat):
    '''
